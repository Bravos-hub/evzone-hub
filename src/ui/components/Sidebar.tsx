...
function Icon({ name, className }: { name?: string; className?: string }) {
  ...
  switch (name) {
    ...
    case 'settings':
      return <svg {...props}><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 01-2.83 2.83l-.06-.06A1.65 1.65 0 0015 19.4a1.65 1.65 0 00-1 .6 1.65 1.65 0 00-.33 1V21a2 2 0 01-4 0v-.09a1.65 1.65 0 00-.33-1 1.65 1.65 0 00-1-.6 1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.6 15a1.65 1.65 0 00-.6-1 1.65 1.65 0 00-1-.33H3a2 2 0 010-4h.09a1.65 1.65 0 001-.33 1.65 1.65 0 00.6-1 1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06A1.65 1.65 0 009 4.6a1.65 1.65 0 001-.6 1.65 1.65 0 00.33-1V3a2 2 0 114 0v.09a1.65 1.65 0 00.33 1 1.65 1.65 0 001 .6 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06A1.65 1.65 0 0019.4 9c.26.3.45.66.55 1.05.1.39.11.8.04 1.2-.07.4-.23.78-.47 1.11-.24.33-.56.6-.93.79z" /></svg>
    default:
      return null
  }
}
...
export function Sidebar({ items: overrideItems, onClose }: SidebarProps) {
  ...
  return (
    <aside className="w-[280px] h-full flex-shrink-0 border-r border-white/5 p-0 bg-bg-secondary flex flex-col overflow-hidden shadow-lg">
      ...
      {/* Navigation */}
      <nav ref={navRef} className="flex flex-col gap-1.5 p-4 flex-1 overflow-y-auto overflow-x-hidden min-h-0 scrollbar-hide">
        {menuItems.map((item) => {
          if (item.divider) {
            return <div key={`divider-${item.path}`} className="h-px bg-white/5 my-1" />
          }
