...
export function SiteOwnerTenants() {
  ...
  const filtered = useMemo(() => {
    ...
    return data
      .filter((record: any) => {
        const recordSite = viewMode === 'applications' ? (record.siteName || record.site?.name) : record.siteName
        return site === 'All' || recordSite === site
      })
      .filter((record: any) => {
        if (viewMode === 'tenants') return type === 'All' || record.type === type
        return true
      })
      .filter((record: any) => {
      ...
      .filter((record: any) => {
        const name = viewMode === 'applications' ? (record.operator?.name || record.operatorId) : record.name
        const itemSite = viewMode === 'applications' ? (record.siteName || record.site?.name) : record.siteName
        const searchStr = `${record.id} ${name} ${itemSite}`.toLowerCase()
  ...
  const kpis = useMemo(() => {
    if (viewMode === 'applications') {
      return {
        ...
        pending: filtered.filter((application: any) => application.status === 'PENDING_REVIEW').length,
      }
    }
    return {
      total: filtered.length,
      active: filtered.filter((tenant: any) => tenant.status === 'Active').length,
      totalEarnings: filtered.reduce((sum: number, tenant: any) => sum + (tenant.earnings || 0), 0),
    }
  }, [filtered, viewMode])
  ...
  const handleUpdateStatus = (id: string, status: 'APPROVED' | 'REJECTED', terms?: any) => {
    ...
    if (status === 'APPROVED' && terms) {
      updateTerms.mutate({ id, message: 'Terms updated' }, {
...
function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${colors[status] || 'bg-gray-500/10 text-gray-500 border border-gray-500/20'}`}>{status}</span>
}
