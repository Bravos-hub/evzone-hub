...
export type SiteForm = {
    ...
    photoFiles: File[]
    photoUrls: string[]
    documentFiles: { title: string; file: File }[]
...
export function AddSite({ onSuccess, onCancel, isOnboarding = false, isFirstSite = false, fullBleed = false }: AddSiteProps) {
    ...
    const handleDocumentUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setForm(prev => ({
            ...prev,
            documentFiles: [...prev.documentFiles, { title: file.name, file }]
        }))
        e.target.value = ''
    }
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-3 pt-4 border-t border-border">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    id="doc-upload"
                                />
                                <label htmlFor="doc-upload" className="btn secondary text-xs cursor-pointer">
                                    + Add Document
                                </label>
                            </div>
                        </div>
                        ...
                        {form.documentFiles.length === 0 ? (
                            <p className="text-xs text-muted italic">No documents added yet (e.g. lease agreements, permits)</p>
                        ) : (
                            <div className="space-y-2">
                                {form.documentFiles.map((doc, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-2 bg-muted/20 rounded-lg border border-border">
