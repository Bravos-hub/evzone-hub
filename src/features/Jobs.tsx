...
export function Jobs() {
  ...
  const filtered = useMemo(() => {
    return jobsData.filter((j) => (statusFilter === 'All' ? true : j.status === statusFilter))
    ...
    <DashboardLayout pageTitle="Jobs">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <div className="card">
          ...
          <div className="text-xl font-bold text-accent">{jobsData.filter((j) => j.status === 'Available').length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-muted">My Active</div>
          <div className="text-xl font-bold text-warn">{jobsData.filter((j) => j.status === 'In Progress' || j.status === 'Accepted').length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-muted">Completed</div>
          <div className="text-xl font-bold text-ok">{jobsData.filter((j) => j.status === 'Completed').length}</div>
        </div>
        <div className="card">
          <div className="text-xs text-muted">Total Earnings</div>
          <div className="text-xl font-bold text-text">
            ${jobsData.reduce((sum, j) => sum + (j.pay || 0), 0).toFixed(2)}
          </div>
        </div>
      </div>
